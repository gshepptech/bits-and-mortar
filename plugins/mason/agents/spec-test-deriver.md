...
- `plugins/mason/agents/` — sibling F2 agents (cross-stream blind)
- `plugins/mason/scripts/` — except your own outputs
- `plugins/blueprint/agents/`, `plugins/blueprint/scripts/` — Drew-side
- `plugins/mason/mcp-server/src/` — MCP server runtime
- Test fixtures, seed data — code-blind extends to all non-spec artifacts in v1

**Allowed read prefixes** (by exception):

- `mill-archive/{run}/spec.md` — your input contract
- `mill-archive/{run}/transcript.md` — citation surface
- `mill-archive/{run}/test_observations/` — your own outputs (writes, never edits)
